...
#Model Setup***************************************************************************
    def Train(self):
        #X and Y train 
         #To numpy arrays 
        xn_train = np.array(self.x_train)
        
        yn_train=np.array(self.y_train)
        ...
        #X and Y test 
         #To numpy arrays 
        xn_test = np.array(self.x_test)
        yn_test = np.array(self.y_test)
    ...
    #Training :
        ...
        #///////////////////////////////////////////
                ...
                test_loss= torch.mean((yt_test - model(xt_test))**2).item()
        ...
        #///////////////////////////////////////////
         ...
         #print Losses
                ...
                print("Epoch: %3i Training loss: %0.2F Test loss: %0.2F"%(epoch,(running_loss/len(train_loader)), test_loss))
        
       #Evaluation
        model.eval()
        with torch.no_grad():
             ypred_train = model(xt_train)
             ypred_test = model(xt_test)
    ...
    def predictionplot(self):
        ...
        self.ui.PlotWidget.axes.plot(self.yn_train, self.ypred_train,'ro')
        self.ui.PlotWidget.axes.plot(self.yn_test,self.ypred_test,'bo')
        self.ui.PlotWidget.axes.plot([self.yn_test().min(),self.yn_test().max()], [self.yn_test.min(), self.yn_test.max()], 'k--', lw=3)
